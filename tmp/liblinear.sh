...
function func_main() {
    ...
    echo -----------start----------- > ${LOG_DIR}/memory_status_`date +%Y-%m-%d_%H:%M:%S`.log

    echo please run > /tmp/liblinear_thrashed
    wait
    echo -----------finish----------- >> ${LOG_DIR}/memory_status_`date +%Y-%m-%d_%H:%M:%S`.log
...
mkdir -p ${results_DIR}
BENCH_BIN=third_party/tmp/liblinear-multicore-2.49
